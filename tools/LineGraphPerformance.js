...
const LineGraph = ({ max_performance_tasks, performance_tasks }) => {
  ...
  useEffect(() => {
    const highest_possible_score = max_performance_tasks.map((value) =>
      parseInt(value)
    );
    const student_score = performance_tasks.map((value) => parseInt(value));
